--[[
    TEAR-LoadScreen validation module (server)
    - GitHub version check
    - GitHub file comparison (excluding media files)
...
-- Skip these path types to avoid false missing-file reports
...
local function generate_validation_key()
    ...
    local key = ""
    for i = 1, 16 do
        -- math.pow is deprecated in Lua 5.4 / CitizenFX, use ^ instead
...
local function validate_resource_name()
    ...
    if current_name ~= VALIDATION.RESOURCE_NAME then
        return false, C.RED .. "[TEAR-LoadScreen Validation Error] Resource name mismatch: " .. current_name .. C.RESET
    end
    return true, C.GREEN .. "Resource name validation passed" .. C.RESET
end

local function validate_author_name(author_from_manifest)
    if not author_from_manifest or author_from_manifest == "" then
        return false, C.RED .. "[TEAR-LoadScreen Validation Error] Missing author in fxmanifest.lua" .. C.RESET
    end
    if author_from_manifest ~= VALIDATION.AUTHOR_NAME then
        return false, C.RED .. "[TEAR-LoadScreen Validation Error] Author mismatch: " .. author_from_manifest .. C.RESET
    end
    return true, C.GREEN .. "Author validation passed" .. C.RESET
end
...
local function validate_version(client_version)
    ...
    -- Compare semantic versions by segment, returns -1/0/1
    ...
    if cmp < 0 then
        -- If major gap is greater than 1, require forced update
        local p_client   = parse_version(client_version)
        local p_required = parse_version(required)
        local forced     = (p_required[1] - p_client[1]) > 1

        if forced then
            return false, C.RED .. "[TEAR-LoadScreen Forced Update] Version too old! Current: " .. client_version .. " -> Minimum: " .. required .. C.RESET
        else
            return false, C.YELLOW .. "[TEAR-LoadScreen Version Warning] Version too old! Current: " .. client_version .. " -> Minimum: " .. required .. C.RESET
        end
    elseif cmp > 0 then
        return true, C.GREEN .. "Version validation passed (newer version detected)" .. C.RESET
    else
        return true, C.GREEN .. "Version validation passed" .. C.RESET
    end
end
...
-- Safely wait for PerformHttpRequest callback inside coroutine
local function fetch_url(url)
    -- This function must be called inside a CreateThread coroutine
    ...
    -- Wait up to 8 seconds, checking periodically
    local waited = 0
...
local function compare_files_with_github()
    ...
    for _, filepath in ipairs(github_files) do
        ...
        -- Skip known media directories without extensions
        elseif ext == "" and SKIP_PATHS[filepath] then
            result.media_excluded = result.media_excluded + 1
        -- Skip .txt/.md placeholder docs (non-functional files)
        elseif SKIP_EXTENSIONS[ext] then
            ...
            if not local_file_exists(filepath) then
                ...
                add_suspicious(C.YELLOW .. "Missing file: " .. filepath .. C.RESET)
            end
        end
    end
    ...
    for _, filepath in ipairs(manifest_files) do
        if not is_media_file(filepath) and not github_map[filepath] then
            ...
            add_suspicious(C.YELLOW .. "Potential extra file: " .. filepath .. C.RESET)
        end
    end
...
local function print_header()
    ...
    print(C.CYAN .. "║" .. C.WHITE .. "  " .. C.GREEN .. "TEAR-LoadScreen" .. C.WHITE .. "          " .. C.WHITE .. "Version: " .. C.CYAN .. "v" .. VALIDATION.REQUIRED_VERSION .. C.WHITE .. "          ║" .. C.RESET)
    print(C.CYAN .. "║" .. C.WHITE .. "  " .. C.YELLOW .. "Author: TEARLESSVVOID" .. C.WHITE .. "                                             ║" .. C.RESET)
...
local function print_result()
    if #VALIDATION_STATE.ERRORS > 0 then
        print("")
        print(C.RED .. "╔════════════════════════════════════════════════════════════╗" .. C.RESET)
        print(C.RED .. "║" .. C.WHITE .. "  ❌ Validation failed - resource disabled" .. string.rep(" ", 19) .. "║" .. C.RESET)
        print(C.RED .. "╠════════════════════════════════════════════════════════════╣" .. C.RESET)
        ...
        print(C.YELLOW .. "║" .. C.WHITE .. "  Please restore original config and retry" .. string.rep(" ", 13) .. "║" .. C.RESET)
        print(C.RED .. "╚════════════════════════════════════════════════════════════╝" .. C.RESET)
        print("")
    else
        if #VALIDATION_STATE.SUSPICIOUS > 0 then
            print("")
            print(C.YELLOW .. "╔════════════════════════════════════════════════════════════╗" .. C.RESET)
            print(C.YELLOW .. "║" .. C.WHITE .. "  ⚠️ Validation passed - suspicious changes found" .. string.rep(" ", 12) .. "║" .. C.RESET)
            print(C.YELLOW .. "╠════════════════════════════════════════════════════════════╣" .. C.RESET)
            for _, info in ipairs(VALIDATION_STATE.SUSPICIOUS) do
                print(C.YELLOW .. "║" .. C.WHITE .. "  " .. info .. string.rep(" ", 50 - #info) .. "║" .. C.RESET)
            end
            print(C.YELLOW .. "╠════════════════════════════════════════════════════════════╣" .. C.RESET)
            print(C.WHITE .. "║" .. C.GREY .. "  Contact the author if you need help" .. string.rep(" ", 19) .. "║" .. C.RESET)
            print(C.YELLOW .. "╚════════════════════════════════════════════════════════════╝" .. C.RESET)
            print("")
        else
            print("")
            print(C.GREEN .. "╔════════════════════════════════════════════════════════════╗" .. C.RESET)
            print(C.GREEN .. "║" .. C.WHITE .. "  ✅ Validation passed - all checks successful!" .. string.rep(" ", 13) .. "║" .. C.RESET)
            print(C.GREEN .. "╚════════════════════════════════════════════════════════════╝" .. C.RESET)
            print("")
        end
    end
...
local function perform_validation()
    ...
    print(C.WHITE .. "║" .. C.CYAN .. "  Starting validation..." .. string.rep(" ", 33) .. "║" .. C.RESET)
    print(C.WHITE .. "╠════════════════════════════════════════════════════════════╣" .. C.RESET)
    ...
    if manifest_path then
        ...
        if author_match then
            ...
            local author_msg = C.RED .. "Failed to parse author" .. C.RESET
            print(C.WHITE .. "║" .. C.RED .. "  ✗ [2/4] " .. author_msg .. string.rep(" ", 45 - #author_msg) .. "║" .. C.RESET)
            add_error(C.RED .. "[TEAR-LoadScreen Validation Error] Failed to parse author from fxmanifest.lua" .. C.RESET)
        end
    else
        local author_msg = C.RED .. "Failed to load fxmanifest.lua" .. C.RESET
        print(C.WHITE .. "║" .. C.RED .. "  ✗ [2/4] " .. author_msg .. string.rep(" ", 45 - #author_msg) .. "║" .. C.RESET)
        add_error(C.RED .. "[TEAR-LoadScreen Validation Error] Failed to load fxmanifest.lua" .. C.RESET)
    end
    ...
    if not version_valid then
        -- Outdated versions are warnings only, not fatal errors
        add_suspicious(version_msg)
        VALIDATION_STATE.VERSION_OUTDATED = true
    end

    print(C.WHITE .. "║" .. C.CYAN .. "  🔍 [4/4] Running GitHub file comparison..." .. string.rep(" ", 15) .. "║" .. C.RESET)
    local compare_result = compare_files_with_github()

    if not compare_result.skipped then
        print(C.WHITE .. "╠════════════════════════════════════════════════════════════╣" .. C.RESET)
        local file_msg = "  GitHub file comparison: " .. compare_result.checked .. "/" .. compare_result.total .. " (excluded " .. compare_result.media_excluded .. " media files)"
        ...
        if #compare_result.missing == 0 and #compare_result.extra == 0 then
            local success_msg = "  ✅ All files match"
    ...
    if VALIDATION_STATE.VERSION_OUTDATED and VALIDATION_STATE.UPDATE_INFO then
        ...
        if info.body and info.body ~= "" then
            print("")
            print(C.YELLOW .. "╔════════════════════════════════════════════════════════════╗" .. C.RESET)
            print(C.YELLOW .. "║" .. C.WHITE .. "  📦 Version changelog" .. string.rep(" ", 35) .. "║" .. C.RESET)
            print(C.YELLOW .. "╠════════════════════════════════════════════════════════════╣" .. C.RESET)
            local latest_msg = "  Latest version: v" .. (info.version or "")
            print(C.WHITE .. "║" .. C.GREEN .. latest_msg .. string.rep(" ", 50 - #latest_msg) .. "║" .. C.RESET)
            local url_msg = "  Download URL: " .. (info.url or "")
            ...
            if #lines > 8 then
                local more_msg = "  ... (see GitHub for full details)"
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    -- Run after a 2s delay in coroutine to keep fetch_url waits safe
